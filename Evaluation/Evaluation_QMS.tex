...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.4\linewidth]{../resources/figures/QMS_Beispiel-Spektren/QMS_Beispiel-Spektren-2.png}
...
\subsubsection{Comparison of spectra for different resolution settings}
The mass-to-charge spectrum of air was analyzed using five different resolution settings. An overlay of the five spectra is shown in Figure~\ref{fig:resolution}. Higher resolution measurements exhibit much narrower and smaller intensity peaks than measurements with lower resolution settings. The selection of peaks and the determination of their Full Width at Half Maximum (FWHM) were performed computationally to ensure consistent baseline handling across different sensitivity ranges. Quantitatively, we compared the Resolving Power and peak width of each measurement, as shown in Table~\ref{tab:fwhm_results}.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.9\linewidth]{../resources/task_5_5_resolution_overlay.png}
  \caption{Comparison of the air mass-to-charge spectrum for different resolution values.}\label{fig:resolution}
\end{figure}

The values from Table~\ref{tab:fwhm_results} were plotted for linear analysis (Figure~\ref{fig:res_trend}). Note that the argon peaks at 40 Th were excluded from the trend analysis, as their weak intensity resulted in a low signal-to-noise ratio, making the FWHM determination unreliable.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.9\linewidth]{../resources/task_5_5_resolution_trend.png}
  \caption{Comparison of mass resolution trends. Lower resolution settings (Res 2, 3) exhibit a strong slope ($\Delta m$ increases with $m$), indicating constant $U/V$ ratio operation. High resolution settings (Res 6) show a flatter trend, indicating constant peak width operation.}\label{fig:res_trend}
\end{figure}

\subsection{Acceleration-Voltage Analysis}
\subsubsection{Influence of Ion Velocity on Resolution}
The acceleration voltage $U_B$ controls ion velocity, establishing an inverse relationship between resolution and transmission. Lower velocities increase the transit time, subjecting ions to more RF field oscillations; this improves the filtering of unstable trajectories and sharpens peaks, albeit with reduced signal intensity. Conversely, higher velocities allow unstable ions to reach the detector, causing peak broadening. Notably, unlike altering the DC/RF ratio, varying $U_B$ affects peak width without shifting peak positions.

To quantify this effect, the ion velocity $v$ can be derived from the kinetic energy provided by the acceleration voltage:
\begin{equation}
  \frac{1}{2}mv^2 = e U_B \implies v = \sqrt{\frac{2e U_B}{m}}
\end{equation}
The separation capability of the quadrupole is directly related to the number of oscillations $N$ an ion experiences within the RF field. This is given by the product of the RF frequency $\nu$ and the flight time $t$:
\begin{equation}
  N = \nu t = \nu \frac{l}{v} = \nu l \sqrt{\frac{m}{2e U_B}}
\end{equation}
where $l$ is the length of the rod system and $\nu$ is the frequency\cite{QMSAnleitung}. This relationship ($N \propto 1/\sqrt{U_B}$) mathematically confirms that lower acceleration voltages increase the number of oscillations $N$, thereby improving the filtering efficiency and resolution as unstable ions are more effectively removed.

\subsubsection{Comparison of spectra for different field axis potentials}
The acceleration voltage $U_B = U_{FR} - U_{FA}$ was varied by adjusting the Field Axis potential ($U_{FA}$) while maintaining a constant Formation Region potential ($U_{FR} \approx 113.1$\,V).

Figure~\ref{fig:acceleration} shows the overlay of air spectra recorded at different acceleration voltages. The data demonstrates a clear trade-off between resolution and transmission: lower acceleration voltages (e.g., $U_B \approx 0.9$\,V) result in significantly sharper peaks (higher resolution) but reduced signal intensity. Conversely, higher acceleration voltages increase ion transmission but lead to substantial peak broadening.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.9\linewidth]{../resources/task_5_6_acceleration_overlay.png}
  \caption{Comparision of different $U_{FA}$ voltages for the air spectra measurements at normalized pressure. The effect of the electron acceleration in the signal is clearly visible, with lower $U_B$ values correspond to higher resolution (narrower peaks) but lower signal intensity.}\label{fig:acceleration}
\end{figure}

To quantitatively evaluate the dependence of the resolution on the ion energy, the FWHM ($\Delta m$) of the $N_2^+$ and $O_2^+$ peaks was extracted for each acceleration voltage $U_B$. For each setting, the number of RF oscillations $n$ performed by the ions was calculated using the relationship $n = \nu \cdot l / v_z$, where $l=0.1$\,m and $\nu=2.5$\,MHz.

The results are plotted in Figure~\ref{fig:dm_vs_n}. The data shows a clear inverse relationship: as the number of oscillations increases (corresponding to lower ion energy), the peak width $\Delta m$ decreases significantly. This confirms that ions spending more time in the quadrupole field undergo more effective mass filtering, resulting in higher resolution.

\begin{table}[H]
  \centering
  \begin{tabular}{cc|cc|cc}
    \toprule
    \multicolumn{2}{c|}{\textbf{Voltage Settings}} & \multicolumn{2}{c|}{\textbf{$N_2^+$ ($28$ Th)}} & \multicolumn{2}{c}{\textbf{$O_2^+$ ($32$ Th)}} \\
    $U_{FA}$ [V] & $U_B$ [V] & $\Delta m$ [Th] & $n$ & $\Delta m$ [Th] & $n$ \\
    \midrule
    112.9 & 0.92   & 0.5347 & 99.02 & 0.5265 & 105.86 \\
    98.0  & 15.83  & 0.5186 & 23.94 & 0.5468 & 25.59  \\
    83.0  & 30.83  & 0.5656 & 17.15 & 0.5891 & 18.34  \\
    63.0  & 50.83  & 0.7100 & 13.36 & 0.7068 & 14.28  \\
    43.0  & 70.83  & 0.8312 & 11.32 & 0.8437 & 12.10  \\
    23.0  & 90.83  & 0.9284 & 9.99  & 1.0143 & 10.68  \\
    3.0   & 110.83 & 1.0657 & 9.05  & 1.2754 & 9.67   \\
    \bottomrule
  \end{tabular}
  \caption{Dependence of peak width ($\Delta m$) and number of oscillations ($n$) on the acceleration voltage ($U_B$) for $N_2^+$ and $O_2^+$. The data demonstrates the inverse relationship between ion energy and mass resolution.}\label{tab:fwhm_n_results}
\end{table}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.9\linewidth]{../resources/task_6_resolution_trend.png}
  \caption{Relationship between the peak width $\Delta m$ (FWHM) and the number of RF oscillations $n$ for $N_2^+$ and $O_2^+$.}\label{fig:dm_vs_n}
\end{figure}

% --- Discussion ---
\section{Discussion}
\subsection{Background Subtraction and Argon Spectrum}
The analysis of the Argon spectrum shows the importance of background subtraction in mass spectrometry. The raw data exhibited a significant water vapor signal ($H_2O^+$, 18 Th), which is a common residual in vacuum systems in chamber walls. The subtraction procedure successfully isolated the Argon signal, but the peak at 28 Th ($N_2^+$) suggests a slight variance in the nitrogen partial pressure between the background and sample measurements.

Notably, the spectrum shows activity in the 36 Th range. Natural argon consists of three stable isotopes: $^{40}Ar$ (99.6\%), $^{36}Ar$ (0.33\%), and $^{38}Ar$ (0.06\%). While the dominant 40 Th peak was clearly resolved, the detection of signals near 36 Th indicates the instrument is well capable to detect trace isotopes.

\subsection{Organic Spectra and Fragmentation Patterns}
For Acetone ($CH_3COCH_3$), the spectrum was characterized by the expected cleavage of methyl groups, resulting in dominant fragments at 43 Th ($CH_3CO^+$) and 15 Th ($CH_3^+$). For Ethanol ($C_2H_5OH$), the characteristic peak at 31 Th ($CH_2OH^+$) confirmed the presence of a primary alcohol, formed by the loss of a methyl group.

The comparison of the measured spectra with reference data shows the correctness of the measurement, although with `unexpected' overlaps. The presence of peaks at 28 Th ($N_2^+$) and 32 Th ($O_2^+$) in the organic spectra confirms that air background impurities remains a constant interference factor. Furthermore, remnants of Argon (40 Th) from previous flushing cycles were observed, showing the difficulty of purging the gas lines in this specific setup.

\subsection{Sensitivity Limits and Air Composition}
The Air spectrum presented a discrepancy regarding the minor components. While $N_2^+$ and $O_2^+$ were identified with high intensity, the peaks for Argon (40 Th, $\approx$0.9\% concentration) and $CO_2$ (44 Th, $\approx$0.04\%) were largely lost in the noise. As expected, the signal loss due to the dynamic range and resolution settings was too high in those peaks.

\subsection{Resolution and Acceleration Voltage}
The resolution setting (Res 2--6) adjusts the slope of the working line in the stability diagram. Lower resolution settings widen the stability region, increasing ion transmission at the cost of peak sharpness. This loss of precision causes neighboring peaks to merge, as observed with $H_2O^+$ ($18$ Th) and its fragment $OH^+$ ($17$ Th). Conversely, high resolution settings (Res 6) provide excellent separation but significantly reduce signal intensity, causing trace components to vanish into the noise floor.

The acceleration voltage ($U_B$) significantly impacts the effective resolution by determining the ion velocity and, consequently, the number of filtering oscillations. At low voltages ($U_B \approx 0.9$ V), ions undergo many oscillations, resulting in high selectivity (sharp peaks) but poor transmission due to entrance losses. As $U_B$ increases, transmission improves, but the filtering efficiency drops. At high voltages ($>50$ V), ions traverse the quadrupole too quickly to be unstable, causing peaks to broaden into undefined plateaus (Figure~\ref{fig:acceleration}). An intermediate voltage of $U_B \approx 15$ V provided the optimal balance between signal strength and mass separation.

% --- Conclusion ---
\section{Conclusion}
Through the analysis of noble gases, atmospheric air, and organic solvents, we can conclude that this specific mass spectrometer setup is a robust tool for mass and molecular structure analysis. It is very capable of resolving isotopes and characteristic fragmentation patterns.

The qualitative analysis of Acetone and Ethanol showed that electron impact ionization induces predictable molecular fragmentation, consistent with standard reference spectra, and the importance of background substraction for isolating signal peaks from residual water vapor and other contaminants. However, the analysis of atmospheric air revealed the sensitivity limitations, as trace components like Argon and $CO_2$ were suppressed by the noise floor when optimizing for the dominant Nitrogen signal.

The different quadrupole field parameters settings confirmed two fundamental operating principles: First, the variation of the resolution setting showed trade-off between mass resolution and transmission efficiency; high-resolution settings provided excellent peak separation but significantly reduced sensitivity, limiting the detection of minor isotopes. Second, the acceleration voltage series experimentally verified the dependence of mass filtering on the ion transit time. The measured peak widths ($\Delta m$) exhibited a clear correlation with the number of RF oscillations ($n$), following the theoretical $\Delta m \propto n^{-2}$ behavior. This confirms the link between the filtering selectivity of a QMS and the number of interactions between the ions and the quadrupole field.

Overall, the QMS proved to be an effective instrument for determining gas composition, if the user carefully balances the trade-offs between resolution, scan speed, and ion energy to match the specific analytical requirements.
