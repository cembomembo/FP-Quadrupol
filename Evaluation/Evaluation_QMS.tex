...
\subsection{Argon Spectrum (with Background Subtraction)}
\begin{itemize}
  \item Plot the argon spectrum and normalize it to the total chamber pressure
  \item \textit{Pressure normalization:} multiply the full intensity axis by a constant factor so that the plotted signal becomes $I/p$ (e.g.\ $I_\mathrm{norm}(m/z)=I(m/z)/p_\mathrm{end}$), i.e.\ divide every intensity value in the spectrum by the measured total pressure of that scan (the $p$ value in the filename)
  \item Assign the main argon-related peaks (Ar$^+$, possible Ar$^{2+}$) and visible impurities, and compare qualitatively to the provided reference spectrum
  \item Background subtraction: scale the residual-gas spectrum to match a chosen reference peak (e.g.\ H$_2$O$^+$) and subtract it from the argon spectrum, then replot and describe which features were removed or improved
\end{itemize}

\subsection{Acetone Spectrum}
\begin{itemize}
  \item Plot the acetone spectrum (optionally focus on the relevant range, e.g. $m/z\approx 10$ to $60$)
  \item Normalize to pressure and assign the dominant parent/fragment peaks using the expected fragmentation pattern
  \item Compare to the provided reference spectrum and discuss unexpected peaks or overlaps (e.g. air/water background, argon remnants)
\end{itemize}

\subsection{Ethanol Spectrum}
\begin{itemize}
  \item Plot the ethanol spectrum and normalize to pressure
  \item Assign the dominant fragment groups (C$_2$H$_x$O$^+$ family etc) and note overlaps with common background peaks
  \item Compare to the provided reference spectrum and comment on differences (e.g. remaining rest-gas contribution)
\end{itemize}

\subsection{Air Spectrum}
\begin{itemize}
  \item Plot the air spectrum and normalize to pressure
  \item Assign the main air peaks (N$_2^+$ at 28, O$_2^+$ at 32, Ar$^+$ at 40, CO$_2^+$ at 44, plus fragments)
  \item Compare to the provided reference spectrum and comment on contamination/background signatures
\end{itemize}

\subsection{Resolution Series in Air (VAR: 6, 5, 4, 3, 2)}
\begin{itemize}
  \item Overlay the air spectra for resolution settings VAR = 6 $\rightarrow$ 2 and discuss the qualitative trade-off (peak narrowing vs signal loss)
  \item For selected peaks, determine the FWHM $\Delta m$ and compile a table of $(m,\Delta m)$
  \item Plot $m$ versus $\Delta m$ as requested and extract/compare resolving power trends
\end{itemize}

\subsection{Acceleration-Voltage Series in Air (Variation of $U_\mathrm{FA}$)}
\begin{itemize}
