...
Task 5: discuss the qualitative trade-off (peak narrowing vs signal loss). Please indicate that the H2O has also the OH+ subpeaks and that theres some error on it. Comment on the missing peaks on the air spectrum (task 4) and how then in task 5 again we could barely get good data in that range of 40-45Th.

- air spectrum 40 and 44 line drowns in noise
- most peaks could be identified but some remain unknown
- partial pressure is only an approxmation, since

\subsection{Resolution}

The resolution setting (Res 2-6) alters the slope of the working line in the Mathieu stability diagram (see fig. \ref{fig:qmf_stability_demtroeder}), thus changing how many Ions are detected for each peak. The results in \ref{fig:resolution} do match very well, with what is expected. For lower resolutions more Iones are detected thus enlargening the peaks, but they also become wider and less precise, also the center of the peaks shifts to the left, which can also be deduced from the stability diagram.
It does however also become clear, that the measurement in H2O(18) is not very precise for the low resolution because it mixes with the peaks of O+(16) and OH+ (17). But also the peak at 33 is almost disappearing in the noise for the high resolution measurement. Suggesting that there is a trade-off to a high resolution setting, this could be countered by noisreduction methodes as the ones that have also been utilized for the other spectra, but in general this is something to keep in mind. 

The resolution also alters when the acceleration voltage in changed. This can be seen in \ref{fig:acceleration}. For low acceleration volotages the Ions have not enough momentum to make it through the spectrometer. This can be seen for the $U_{B} = 0.2 \: V$ measurment where hardly any Ions are detected, because the acceleration voltage so low, the Ions that make it into and through the spectrometer are largely just entering by chance.
For higher acceleration voltages however the Ions make it through. In the present measurements an acceleration voltage of $U_B = 15 \: V$ seems to already be very sufficient to clearly accelerate the Ions through the spectrometer. As the acceleration voltage is increased more and more Ions pass all the way through, even the ones that would not have been stable in the spectrometer if they had had a lower velocity. This is why for large $U_B$ the peaks are widened. Also resulting in a lower resolution, which becomes very clear for the peaks below 20 Th, that merge into one plateau for $U_B = 110 \: V$. 
