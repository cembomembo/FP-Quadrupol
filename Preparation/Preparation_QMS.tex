...
% --- Title page ---
\title{\textbf{Quadrupole Mass Spectrometer}}
\author{
  \\Preparation Report \\\\\\\\\\\\
  \textbf{Cem Boyaci} \\
...
To ensure that there are no other gases in the pipes of the gas inlet and to minimize pollution a third vacuum pump a  \textbf{Rotary Vane Pump} is used. This creates a fore-vacuum in the pipes that is then flooded with the gas that is to be measured.  
...
\subsection{Detector}
After passing through the quadrupole mass filter, the ions that maintain stable trajectories are directed towards the detection system. The primary component for ion detection in this setup is a Secondary Electron Multiplier.

The detection process follows this signal chain:
\begin{itemize}
  \item \textbf{Secondary Electron Multiplier (Multiplier):} This device serves as the initial detector. It amplifies the weak ion current generated by the impinging ions through a cascade of secondary electron emission, allowing for the detection of very low partial pressures.
  \item \textbf{Electrometer Amplifier:} The current output from the multiplier (or the direct ion current if the multiplier is not active) is fed into an electrometer amplifier. This unit converts the low-level electrical current into a voltage signal proportional to the ion intensity.
  \item \textbf{Signal Transmission:} The amplified signal labeled as `Intensität' (Intensity) is transmitted from the QMS Control Unit to the PC interface for recording and analysis via the LabVIEW software.
\end{itemize}

During the measurement, the sensitivity of the electrometer can be adjusted (e.g., to ranges such as $10^{-11}$ or $10^{-12}$\,A) to accommodate peaks of varying intensity.

\subsection{Pressure Measurements}
Continuous monitoring of the pressure is essential for the operation of the mass spectrometer, both to ensure the safety of the components and to provide a reference for the quantitative analysis of the gas spectra.

The pressure measurement system consists of the following components:
\begin{itemize}
  \item \textbf{Recipient Pressure Gauge:} The pressure within the main vacuum chamber is measured using a full-range measuring head operating on the cold cathode ionization principle. This gauge is capable of measuring the high vacuum required for the experiment (typically in the range of $10^{-5}$\,mbar to $10^{-7}$\,mbar).
  \item \textbf{Gas Inlet Manometer:} A separate manometer is installed on the gas supply line to monitor the pressure of the test gas before it enters the dosing valve.
\end{itemize}

During the experiment, the pressure reading from the cold cathode gauge serves two critical functions:
\begin{enumerate}
  \item \textbf{Safety Interlock:} It ensures that the pressure is below the critical threshold ($p < 5 \times 10^{-5}$\,mbar) before the emission current of the QMS filament is activated, preventing oxidation and damage to the hot cathode.
  \item \textbf{Normalization:} The total pressure recorded during each scan is used to normalize the ion intensity data, allowing for the conversion of relative peak heights into partial pressures for component assignment.
\end{enumerate}
\newpage
% --- Procedure ---
\section{Procedure}

\subsection{Preparation and Starting of the Device}

First the pressure in the recipient has to be smaller than $5 \times 10^{-5}$ mbar, this is done using the two vacuum pumps described in section \ref*{sec:setup}. Then the devices can be turned on in the following order:
\begin{itemize}
  \item QMS-Control device (with emission turned OFF!)
  \item External Voltage-sources (on 200 V and 100 V)
  \item Emission is turned ON, and slowly increased to 0.2
\end{itemize}
The following other voltages are set: \begin{itemize}
  \item $U_{FR} = 113 \: V$  (Formation Region)
  \item $U_{KA} = 0 \: V$  (Cathode)
  \item $U_{FA} = 106 \: V$   (Field Axis)
\end{itemize}
The heating filament is turned on, this increases the pressure, it takes 30 to 40 minutes for the vacuum to be at its initial value again.

\subsection{Dosage of the Test-Gas}
\label{sec:dosage}
Before any gas is let into the recipient the emission has to be turned off again.

Then the supply lines up to the dosage valve are pumped out using the rotary vane pump and then flooded with the test gas. This is repeated twice. 

Now the test gas can be let in through the dosage valve. This is done slowly until the pressure stabilizes around $1\times 10^{-5}$ mbar. (The pressure should not increase above $5\times 10^{-4}$ mbar, this could damage the vacuum pumps)

Then the emission current can be turned on again and slowly increased to 0.2 mA.
\subsection{Measurement of the Mass Spectrum}
The mass spectrum is then measured on the computer with a LabView programm. The resolution of the quadrupole mass spectrometer is set on CAL, and thus calibrated.
The spectrum is then normalized to the pressure of the recipient, so that the peak intensities are corresponding to the partial pressure of the molecule or atom that is being measured.
Then the different mass peaks are assigned different ionized fragments as is explained in section \ref{sec:massspectra}.
These steps and the ones from section \ref{sec:dosage} are repeatead for each gas that is measured, inlcuding air. 


\subsection{Variation of the Resolution}
To determine the resolution power of the spectrometer different spectra are recorded with different resolutions starting with the highest.
In this case the resolution is set to VAR and varied from 2 to 6. That is later analyzed by plotting $m$ against $\Delta m$. 

The resolution $m/\Delta m$ is then also examined as a function of the acceleration voltage $U_A = U_{FC} - U_{FA}$. This is done first with the smallest acceleration voltage, which is then slowly increased. For this $U_{FA}$ is decreased from 110 to 0 V.
The resulting spectra will also then be analyzed later, by plotting $\Delta m$ against the number of onszillations an ion went through while passing the electric field of the QMS rods. 

\subsection{Turning off of the Device}
When all measurements are done the device is turned of by first switching off the emission, then the external voltage sources and lastly the QMS control device. 
\cite{QMSAnleitung}
